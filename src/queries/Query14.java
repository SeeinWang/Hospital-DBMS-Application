...
public class Query14 {
	...
	private void initialize() {
		...
		frame = new JFrame();
		frame.setBounds(300,240,448,263);
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);	
		frame.getContentPane().setLayout(null);
		
		JPanel panel = new JPanel();
		panel.setBounds(0, 0, 432, 226);
		...
		JLabel infoLabel = new JLabel("Find the least expensive");
		...
		lblMedicineAvailable.setFont(new Font("Tahoma", Font.PLAIN, 36));
				...
				insert.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent arg0) {
						selectLeastExpensiveMedicine();
					}
				});	
		frame.setTitle("Query14: Find the least expensive medicine");
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setVisible(true);
	}
	
	
	public static int selectLeastExpensiveMedicine() {
		...
		try {
			...
			String select = "select mname from Medicine where price=(select min(price) from Medicine)";
			String res = "";
...
//				res = res.substring(0, res.length() - 2);	// remove last comma
			...
			ps.close();	
			JOptionPane.showMessageDialog(null, String.format("Least expensive medicine is:\n%s", res));
			return 0;
